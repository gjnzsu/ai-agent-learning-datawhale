...
- [ ] 能用一句话区分 MCP、A2A 和 ANP。
- [ ] 能解释 MCP Host、Client、Server 的关系。
- [ ] 能解释 A2A Agent Card、Task 和 Artifact。
- [ ] 能区分用户、应用、Agent 和下游服务身份。
- [ ] 能说明为什么工具 Schema 不等于权限。
- [ ] 能说明预授权与具体调用授权的区别。
- [ ] 能为 KYC Agent 设计最小权限委派链路。
- [ ] 能列出至少五项协议接入治理措施。
